...
type BJJ = { id:string; performed_at:string; duration_min:number; kind:'class'|'drilling'|'open_mat' }
type Profile = {
  weekly_goal:number|null; target_weeks:number|null; goal_start:string|null;
...
function weekKey(d: Date){ return startOfWeekSunday(d).toISOString().slice(0,10) }
...
export default function Dashboard(){
  ...
  const strengthStreak = useMemo(()=>{
    ...
    for (let i=0;i<120;i++){ keys.push(cursor.toISOString().slice(0,10)); cursor = new Date(cursor); cursor.setDate(cursor.getDate()-7) }
    let streak = 0
    for (const k of keys){
  ...
  // -------- BJJ weekly counts/minutes & streak --------
      ...
      <main className="max-w-4xl mx-auto p-4 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Link href="/workouts/new" className="btn">Start workout</Link>
            <Link href="/jiu-jitsu" className="toggle">Log Jiu Jitsu</Link>
          </div>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div className="card">
            <div className="flex items-start justify-between">
              <div>
                <div className="text-sm text-white/70">Strength — Weekly consistency</div>
                <div className="text-3xl font-semibold">{strengthStreak} week{strengthStreak===1?'':'s'}</div>
                <div className="text-white/70 text-sm mt-1">
                  This week: <span className="text-white">{thisWeekCount}</span>/<span className="text-white">{weeklyGoal}</span> {onTrackStrength ? <span className="text-green-400">• on track</span> : <span className="text-brand-red">• catch up</span>}
                </div>
              </div>
              <Link href="/settings" className="toggle">Edit goal</Link>
            </div>
          ...
          <div className="card">
            <div className="flex items-start justify-between">
              <div>
                <div className="text-sm text-white/70">Jiu Jitsu — Weekly consistency</div>
                <div className="text-3xl font-semibold">{bjjStreak} week{bjjStreak===1?'':'s'}</div>
                <div className="text-white/70 text-sm mt-1">
                  This week: <span className="text-white">{bjjThisWeekCount}</span>/<span className="text-white">{bjjWeeklyGoal}</span> {onTrackBjj ? <span className="text-green-400">• on track</span> : <span className="text-brand-red">• catch up</span>}  •  Mat time: <span className="text-white">{bjjThisWeekMin}</span> min
                </div>
              </div>
              <Link href="/settings" className="toggle">Edit goal</Link>
            </div>
            ...
            <div className="grid gap-2">
              {recentW.map(w=>(
                <div key={w.id} className="flex items-center justify-between">
                  <div className="text-white/90">{new Date(w.performed_at).toLocaleDateString()} — {w.title ?? 'Untitled'}</div>
                  <Link href={`/history?highlight=${w.id}`} className="toggle">Open</Link>
                </div>
              ))}
              {!recentW.length && <div className="text-white/60">No workouts yet.</div>}
            </div>
          </div>
          <div className="card">
              ...
              {recentB.map(s=>(
                <div key={s.id} className="flex items-center justify-between">
                  <div className="text-white/90">
                    {new Date(s.performed_at).toLocaleDateString()} — {s.kind.replace('_',' ')} • {s.duration_min} min
                  </div>
                </div>
              ))}
              {!recentB.length && <div className="text-white/60">No Jiu Jitsu sessions yet.</div>}
            </div>
          </div>
        </div>
